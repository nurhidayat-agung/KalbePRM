 ...
 * edited for migration to
 * android room db by ghqp 27-12-2019
 */


public class Mobile_mConfigBL extends clsMainBL {

    public List<clsMobile_mConfig> getById(int id, Context context) {
        // return new Select().from(clsMobile_mConfig.class).where("idConfig=?",new String[]{String.valueOf(id)}).execute();
        return AppDatabase.getDatabase(context).daoMobileMConfig().getbyIdConfig(id);
    }

    public String getValue(int id, Context context) {
        AppDatabase appDatabase = AppDatabase.getDatabase(context);
        String txtValue = "";
        List<clsMobile_mConfig> ListData = appDatabase.daoMobileMConfig().getbyIdConfig(id);
        if (ListData.size() > 0) {
        ...
        // return clsHardCode.baseUrl;
        return txtValue;
    }

    public List<clsMobile_mConfig> getAll(Context context) {
        return AppDatabase.getDatabase(context).daoMobileMConfig().getAll();
    }
