...
public class ListCallPlanFragment extends Fragment implements IXListViewListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        ListOfDetailCall= new Select().from(clsMobile_trVisitPlan_Detail.class).where(_clsMobile_trVisitPlan_Detail.txtConstintSubmit+"=1").execute();
    ...
    private void viewList(Context ctx, int position) {
        ...
        if(ListDataclsMobile_trVisitPlan_Detail_Item.size()>0){
            ...
            for (clsMobile_trVisitPlan_Detail_Item dt :ListDataclsMobile_trVisitPlan_Detail_Item) {
                ...
                List<clsMobile_mBinaryFile> ListOfclsMobile_mBinaryFile=new Select().from(clsMobile_mBinaryFile.class).where(dtclsMobile_mBinaryFile.txtGUI_IDTable+"=",dt.txtGUI_Detail_Item).execute();
