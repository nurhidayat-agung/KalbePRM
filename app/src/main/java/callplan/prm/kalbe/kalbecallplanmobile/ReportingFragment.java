...
public class ReportingFragment extends Fragment implements IXListViewListener {
    ...
    private void ListData() {
        ...
        // .where(_clsMobile_trVisitPlan_Detail.txtConstintSubmit + " in (2,3)").execute();
        ListOfDetailCall = appDatabase.daoMobileTrVisitPlanDetail()
                .getByIntSubmitLimit(new ArrayList<String>(){{add("2");add("3");}});
    ...
    private void viewList(Context ctx, int position) {
        ...
        List<clsMobile_trVisitPlan_Detail_Item> ListDataclsMobile_trVisitPlan_Detail_Item = appDatabase.daoMobileTrVisitPlanDetailItem()
                .getbytxtConsttxtGUI_DetailOrdertxtConsintNo(dtclsMobile_trVisitPlan_Detail.txtGUI_Detail);
