    ...
    private void ListData() {
        ...
        if (ListOfclsMobile_trVisitPlan_Detail.size() > 0) {
            ...
            for (clsMobile_trVisitPlan_Detail _DtDetail : ListOfclsMobile_trVisitPlan_Detail) {
                ...
                // List<clsMobile_mVisitPlanCategory> ListOfDataclsMobile_mVisitPlanCategory = new Select()
                // .from(clsMobile_mVisitPlanCategory.class)
                // .where(_dtclsMobile_mVisitPlanCategory.txtConstintCategoryID + "=?", _DtDetail.intCategoryID).execute();
                List<clsMobile_mVisitPlanCategory> ListOfDataclsMobile_mVisitPlanCategory = appDatabase.daoMobileMVisitPlanCategory()
                        .getBytxtConstintCategoryID(_DtDetail.intCategoryID);
    ...
    private class AsyncCall extends AsyncTask<JSONObject, Void, JSONObject> {
        ...
        protected JSONObject doInBackground(JSONObject... params) {
            ...
            try {
                ...
                if (ListOfclsMobile_trVisitPlan_Detail.size() > 0) {
                    JsonResult = _clsMainBL.PushDataCallPlan(getActivity());
                    do {
                        JsonResult = _clsMainBL.PushDataCallPlan(getActivity());
...
//                                }
                                ...
                                clsMobile_trUserLogin dtUserLogin = new Mobile_trUserLoginBL().CheckUserActive(getActivity());
                                if (dtUserLogin.txtUserID == null) {
                                    new clsMainBL().DeleteData(getActivity());
